...
class TraficSim(Construct):
    ...
    def __init__(self, scope: Construct, id: str, *, vpc: aws_ec2.IVpc, url: str, tps: int):
        ...
        task_definition.add_container("Pinger",
                                        image = aws_ecs.ContainerImage.from_asset(cls = None, directory = "/aws_examples/url_shortner/pinger"),
                                        environment = {"URL": url}
                                    )

        fargate_service = aws_ecs.FargateService(self, "PingerService", cluster = cluster, task_definition = task_definition, desired_count=tsp)